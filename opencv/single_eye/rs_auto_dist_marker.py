...
''' use realsense camera to locate marker '''
...
def main():
    ...
    try:
        while True:
            ...
            def local_put_text(text, pos, color):
                cv2.putText(color_image, text, pos, cv2.FONT_HERSHEY_SIMPLEX,
                            1.2, color, 3)
            ...
            local_put_text("rs: %4dmm" % dist_from_rs,
                           (color_image.shape[1] - 500, color_image.shape[0] - 60),
                           (0, 255, 255))
            local_put_text("wc: %4dmm" % d,
